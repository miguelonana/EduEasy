...
if(isset($_GET['id']) AND !empty($_GET['id'])){
    ...
    $checkIfUserExists = $bdd->prepare('SELECT pseudo, nom, prenom FROM usersf WHERE id = ?');
    ...
    if($checkIfUserExists->rowCount() > 0){
        ...
        $getHisQuestions = $bdd->prepare('SELECT * FROM question WHERE id_auteur = ? ORDER BY id DESC');
